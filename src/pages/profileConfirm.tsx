...
const ProfileConfirm: React.FC = () => {
  ...
  const { name, barcode, tg } = location.state;
  ...
  const handleLogIn = () => {
    // Here you would typically handle the login logic, e.g., send data to backend
      ...
      <div className="w-full max-w-97.5 flex items-center justify-between px-4 py-3 mt-2.5 mb-5">
        <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-sm">
          <button
            ...
            onClick={() => navigate("/auth", { state: { name, barcode, tg: tg || "@arself" } })}>
            ‹
          </button>
        </div>
      ...
      <div className="bg-white rounded-3xl px-6 py-7 max-w-90 w-[90%] shadow-[0_4px_24px_rgba(0,0,0,0.07)]">
        ...
        <div className="flex flex-col gap-3 mb-5">
          ...
          <div className="flex flex-row gap-3">
            {/* Name field */}
            <div className="flex-6 bg-[#f0f6ff] rounded-2xl p-[14px_16px] relative overflow-hidden min-h-22.5 flex flex-col justify-end">
              ...
              <img
                ...
                alt=""
                className="absolute right-0 -bottom-2.5 w-20 h-25"
              />
            </div>
            {/* BarCode field */}
            <div className="flex-4 bg-[#79c0ff] rounded-2xl p-[14px_16px] relative overflow-hidden min-h-22.5 flex flex-col justify-end">
          ...
          <div className="flex flex-row gap-3">
            ...
            {/* Language field */}
            <div className="flex-6 bg-[#f0f6ff] rounded-2xl p-[14px_16px] relative overflow-hidden min-h-22.5 flex flex-col justify-center items-start">
              <span className="text-base font-bold text-[#080808]">{information.lang}</span>
              <img
                src="./src/assets/profile_confirmation/Info.png"
                alt=""
                className="absolute right-0 bottom-0 w-18.75 h-18.75"
              />
            </div>
          </div>
        </div>
        ...
        {/* Buttons */}
        <button onClick={handleLogIn} className="w-full h-12 bg-[#0088ff] text-white border-none rounded-full text-base font-light cursor-pointer mb-3">
          Все верно, продолжить
        </button>
        <button onClick={() => {navigate("/edit-info", {state: {name, barcode, tg: information.tg_user || "@arself"}})}} className="w-full h-12 bg-[#ecedf0] text-[#374151] border-none rounded-full text-[15px] font-light cursor-pointer">
